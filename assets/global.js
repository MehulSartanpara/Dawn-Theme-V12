...
$(document).ready(function(){
  $(document).on('click','.add-to-cart', function(e){
    e.preventDefault();
    var VarientId = $(this).closest('form').find('.variant__id').val();
    var Data = {
      id: VarientId,
      quantity: 1
    }
...
function channge_quantity_zero() {
    var cartItems = $('.gift_product_in_cart');
  console.log("cartItems" , cartItems)
    var cartUpdates = {};
    cartItems.each(function() {
      var lineItemId = $(this).attr('data-item-id');
      cartUpdates[lineItemId] = 0;
    });
    var formData = {updates: cartUpdates};
...
function mincart(){
  $.ajax({
    ...
    success: function (updated_cart){ 
       $('.drawer__inner').html($(updated_cart).find('.drawer__inner').html()); 
       $('cart-drawer').removeClass('is-empty');
       $.getJSON('/cart.js', function(cart){ 
          if($("div").hasClass("cart-count-bubble"))
          {
  ...
  rangeInput.forEach((input) => {
    input.addEventListener("input", (e) => {
      ...
      if (maxRange - minRange < rangeMin) {     
        ...
        $('#Filter-Price-GTE').val(minRange);
        $('#Filter-Price-LTE').val(maxRange);
...
$(window).on('load', function() {
  $('.range-input .min').val($('#Filter-Price-GTE').val());
  LoadRangeBar();
});
